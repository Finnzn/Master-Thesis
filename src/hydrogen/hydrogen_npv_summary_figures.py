"""Generate hydrogen-sector financial-metric figures and CSV outputs.
...
def _hydrogen_financial_metric_config(financial_metric: str) -> Mapping[str, object]:
    """Return display and export settings for one hydrogen financial metric."""

    if financial_metric not in HYDROGEN_FINANCIAL_METRIC_OPTIONS:
        valid_metrics = ", ".join(HYDROGEN_FINANCIAL_METRIC_OPTIONS)
        raise ValueError(
            f"Unknown financial_metric {financial_metric!r}. "
            f"Use one of: {valid_metrics}."
        )

    return HYDROGEN_FINANCIAL_METRIC_OPTIONS[financial_metric]


def _with_hydrogen_display_labels(ranking_summary):
    """Return a ranking summary copy with human-readable technology labels."""

    return ranking_summary.assign(
        display_label=ranking_summary["technology"].map(HYDROGEN_TECHNOLOGY_LABELS).fillna(
...
def _with_deterministic_retrofit_mode(
    ...
    """Add the hydrogen MC retrofit-mode column to deterministic export rows.

    The deterministic hydrogen model does not need `retrofit_bau_mode` internally,
    but raw and processed hydrogen CSV exports should keep the same column schema as
    Monte Carlo outputs.
    """
...
def hydrogen_npv_distribution_summary_million_eur(
    results_by_technology: Mapping[str, Mapping[str, object]],
    labels: Mapping[str, str] = HYDROGEN_TECHNOLOGY_LABELS,
...
def hydrogen_npv_distribution_summary(
    results_by_technology: Mapping[str, Mapping[str, object]],
    labels: Mapping[str, str] = HYDROGEN_TECHNOLOGY_LABELS,
    ...
    config = _hydrogen_financial_metric_config(financial_metric)
...
def calculate_mean_hydrogen_npv_million_eur(
    sample_size: int = DEFAULT_SAMPLE_SIZE,
    random_seed: int = DEFAULT_RANDOM_SEED,
    technologies: tuple[str, ...] | None = None,
    retrofit_bau_mode: str = DEFAULT_RETROFIT_BAU_MODE,
) -> dict[str, float]:
    """Calculate mean simulated NPV by hydrogen technology in million EUR."""

    return mean_npv_million_eur(
        results_by_item=simulate_hydrogen_results(
            sample_size=sample_size,
            random_seed=random_seed,
            technologies=technologies,
            retrofit_bau_mode=retrofit_bau_mode,
        ),
        labels=HYDROGEN_TECHNOLOGY_LABELS,
    )


def calculate_mean_hydrogen_npv(
    sample_size: int = DEFAULT_SAMPLE_SIZE,
    random_seed: int = DEFAULT_RANDOM_SEED,
    technologies: tuple[str, ...] | None = None,
    retrofit_bau_mode: str = DEFAULT_RETROFIT_BAU_MODE,
    ...
    """Calculate a mean simulated financial metric by hydrogen technology."""
...
def calculate_deterministic_hydrogen_npv_million_eur(
    technologies: tuple[str, ...] | None = None,
) -> dict[str, float]:
    """Calculate deterministic NPV by hydrogen technology in million EUR."""

    return deterministic_npv_million_eur(
        results_by_item=calculate_deterministic_hydrogen_results(
            technologies=technologies
        ),
        labels=HYDROGEN_TECHNOLOGY_LABELS,
    )
...
def calculate_deterministic_hydrogen_npv(
    technologies: tuple[str, ...] | None = None,
    financial_metric: str = "NPV",
) -> dict[str, float]:
    """Calculate a deterministic financial metric by hydrogen technology."""
...
def save_hydrogen_mean_npv_figure(
    output_dir: Path,
    sample_size: int = DEFAULT_SAMPLE_SIZE,
    random_seed: int = DEFAULT_RANDOM_SEED,
    run_date: date | None = None,
    sector_name: str = "Hydrogen",
    retrofit_bau_mode: str = DEFAULT_RETROFIT_BAU_MODE,
    financial_metric: str = "NPV",
) -> Path:
    """Save the simulated mean NPV comparison figure for hydrogen."""

    config = _hydrogen_financial_metric_config(financial_metric)
    results = simulate_hydrogen_results(
        sample_size=sample_size,
        random_seed=random_seed,
        retrofit_bau_mode=retrofit_bau_mode,
    )
    summary = hydrogen_npv_distribution_summary(results, financial_metric=financial_metric)
    ...
    return plot_financial_metric_technology_bars(
        ...
        title=f"Monte Carlo mean {config['ranking_label']} by hydrogen technology",
...
def save_hydrogen_deterministic_npv_figure(
    output_dir: Path,
    run_date: date | None = None,
    sector_name: str = "Hydrogen",
    financial_metric: str = "NPV",
) -> Path:
    """Save the deterministic NPV comparison figure for hydrogen."""

    config = _hydrogen_financial_metric_config(financial_metric)
    values = calculate_deterministic_hydrogen_npv(financial_metric=financial_metric)
    output_path = dated_figure_path(
        output_dir=output_dir,
...
def save_hydrogen_mean_npv_outputs(
    figure_dir: Path,
    raw_data_dir: Path,
    ...
    raw_csv_path = save_results_csv(
        ...
        columns=HYDROGEN_RAW_INPUT_COLUMNS,
    ...
    processed_csv_path = save_results_csv(
        results_by_item=results,
        columns=HYDROGEN_PROCESSED_OUTPUT_COLUMNS,
        output_path=dated_csv_path(
    ...
    if save_ranking_outputs:
        ranking, ranking_summary = calculate_hydrogen_npv_rankings_from_results(
            results=results,
            sector_name=sector_name,
            financial_metric=financial_metric,
        )
        output_paths.extend(
            save_hydrogen_npv_ranking_outputs(
                ranking=ranking,
...
def calculate_hydrogen_npv_rankings_from_results(
    results: Mapping[str, Mapping[str, object]],
    sector_name: str = "Hydrogen",
    financial_metric: str = "NPV",
):
    """Calculate raw and summary NPV rank tables from hydrogen results."""
...
def calculate_hydrogen_npv_rankings(
    sample_size: int = DEFAULT_SAMPLE_SIZE,
    random_seed: int = DEFAULT_RANDOM_SEED,
    technologies: tuple[str, ...] | None = None,
    sector_name: str = "Hydrogen",
    ...
    """Run hydrogen Monte Carlo simulations and return NPV ranking tables."""
    ...
    return calculate_hydrogen_npv_rankings_from_results(
        results=results,
        sector_name=sector_name,
        financial_metric=financial_metric,
    )
...
def save_hydrogen_npv_ranking_outputs(
    ranking,
    ranking_summary,
    figure_dir: Path,
    raw_data_dir: Path,
    ...
    """Save hydrogen NPV ranking CSVs and/or plots."""
    ...
    if save_ranking_plots:
        output_paths.append(
            plot_average_rank_bars(
                ranking_summary=_with_hydrogen_display_labels(ranking_summary),
                output_path=dated_figure_path(
                    output_dir=figure_dir,
...
def generate_hydrogen_npv_rankings(
    ...
    """Return hydrogen NPV ranking DataFrames and optionally save outputs."""

    ranking, ranking_summary = calculate_hydrogen_npv_rankings(
        sample_size=sample_size,
        random_seed=random_seed,
        technologies=technologies,
        sector_name=sector_name,
    ...
    if save_ranking_outputs and (save_ranking_csv or save_ranking_plots):
        root = _project_root()
        output_paths = save_hydrogen_npv_ranking_outputs(
            ranking=ranking,
            ranking_summary=ranking_summary,
...
def save_hydrogen_deterministic_npv_outputs(
    figure_dir: Path,
    raw_data_dir: Path,
    processed_data_dir: Path,
    run_date: date | None = None,
...
def save_hydrogen_npv_outputs(
    ...
    """Save simulated mean and deterministic hydrogen NPV outputs."""

    return (
        *save_hydrogen_mean_npv_outputs(
            figure_dir=figure_dir,
            raw_data_dir=raw_data_dir,
            processed_data_dir=processed_data_dir,
            sample_size=sample_size,
        ...
        *save_hydrogen_deterministic_npv_outputs(
            figure_dir=figure_dir,
            raw_data_dir=raw_data_dir,
            processed_data_dir=processed_data_dir,
            run_date=run_date,
...
def save_hydrogen_npv_figures(
    ...
    """Save both simulated mean and deterministic hydrogen NPV figures."""

    config = _hydrogen_financial_metric_config(financial_metric)
    output_date = run_date or date.today()
    simulated_results = simulate_hydrogen_results(
        sample_size=sample_size,
        random_seed=random_seed,
        retrofit_bau_mode=retrofit_bau_mode,
    )
    simulated_summary = hydrogen_npv_distribution_summary(
        simulated_results,
        financial_metric=financial_metric,
    )
    mean_values = _distribution_stat(simulated_summary, "mean")
    deterministic_values = calculate_deterministic_hydrogen_npv(financial_metric=financial_metric)
...
def parse_args() -> argparse.Namespace:
    """Parse command-line options for regenerating hydrogen outputs."""

    parser = argparse.ArgumentParser(
        description="Generate hydrogen-sector financial-metric comparison figures."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="BAU baseline mode for retrofit hydrogen technologies.",
    )
    parser.add_argument(
        "--kind",
    ...
    parser.add_argument(
        "--metric",
        choices=tuple(HYDROGEN_FINANCIAL_METRIC_OPTIONS),
        default="NPV",
...
def main() -> None:
    """Run the hydrogen output workflow selected from the command line."""
    ...
    if args.no_data:
        if args.kind == "all":
            output_paths = save_hydrogen_npv_figures(
            ...
            output_paths = (
                save_hydrogen_deterministic_npv_figure(
                    output_dir=args.output_dir,
                    sector_name=args.sector_name,
                    financial_metric=args.metric,
                ),
        ...
        output_paths = save_hydrogen_npv_outputs(
            figure_dir=args.output_dir,
            raw_data_dir=args.raw_data_dir,
            processed_data_dir=args.processed_data_dir,
            sample_size=args.sample_size,
        ...
        output_paths = save_hydrogen_mean_npv_outputs(
            figure_dir=args.output_dir,
            raw_data_dir=args.raw_data_dir,
            processed_data_dir=args.processed_data_dir,
            sample_size=args.sample_size,
        ...
        output_paths = save_hydrogen_deterministic_npv_outputs(
            figure_dir=args.output_dir,
            raw_data_dir=args.raw_data_dir,
            processed_data_dir=args.processed_data_dir,
            sector_name=args.sector_name,
