"""Hydrogen marginal abatement cost curve calculations.

The MACC compares each hydrogen technology with the NG-SMR reference. The
abatement potential is direct stack-emissions abatement only:

    NG-SMR direct emissions - technology direct emissions
...
All routes are compared at the same annual hydrogen output, so their plotted
...
DEFAULT_MACC_TECHNOLOGIES = tuple(
    technology
    for technology in HYDROGEN_MACC_TECHNOLOGY_LABELS
    if technology != "ng_smr"
)
...
def deterministic_hydrogen_macc(
    technologies: tuple[str, ...] = DEFAULT_MACC_TECHNOLOGIES,
) -> pd.DataFrame:
    """Calculate deterministic hydrogen MACC values relative to NG-SMR."""

    selected_technologies = ("ng_smr", *technologies)
    results = calculate_deterministic_hydrogen_results(
        technologies=selected_technologies,
    )
    return build_hydrogen_macc_table(results)


def simulated_hydrogen_macc(
    sample_size: int = DEFAULT_SAMPLE_SIZE,
    random_seed: int = DEFAULT_RANDOM_SEED,
    technologies: tuple[str, ...] = DEFAULT_MACC_TECHNOLOGIES,
    retrofit_bau_mode: str = DEFAULT_RETROFIT_BAU_MODE,
) -> pd.DataFrame:
    """Calculate hydrogen MACC values from aligned Monte Carlo simulations.
...
def build_hydrogen_macc_table(
    results_by_technology: Mapping[str, Mapping[str, object]],
) -> pd.DataFrame:
    """Build a hydrogen MACC table from deterministic or simulated result mappings."""

    if "ng_smr" not in results_by_technology:
        raise ValueError("results_by_technology must include 'ng_smr'.")

    reference = results_by_technology["ng_smr"]
        ...
        rows.append(
            {
                ...
                "label": HYDROGEN_MACC_TECHNOLOGY_LABELS.get(technology, technology),
...
def plot_hydrogen_macc(
    macc_table: pd.DataFrame,
    output_path: Path | None = None,
    title: str = "Hydrogen Sector - Marginal Abatement Cost Curve",
) -> object:
    """Plot a hydrogen marginal abatement cost curve."""
    ...
    fig.text(
        0.99,
        0.02,
        "Each width is direct emissions avoided at 0.1 MtH2/year; routes are not additive. Costs include T&S and exclude carbon payments.",
        ha="right",
        fontsize=8,
        color="#555555",
    )
...
def generate_hydrogen_macc_outputs(
    project_root: Path,
    ...
    """Save one hydrogen MACC CSV and PNG figure."""

    table = (
        deterministic_hydrogen_macc(technologies=technologies)
        if deterministic
        else simulated_hydrogen_macc(
            sample_size=sample_size,
            random_seed=random_seed,
    ...
    csv_path = processed_dir / f"{run_date}-Hydrogen_MACC_{mode}.csv"
    table.to_csv(csv_path, index=False)

    resolved_figure_dir = figure_dir or project_root / "figures"
    figure_path = resolved_figure_dir / f"{run_date}-Hydrogen_MACC_{mode}.png"
    fig = plot_hydrogen_macc(
        table,
        output_path=figure_path,
        title=f"Hydrogen Sector - Marginal Abatement Cost Curve ({mode.lower()})",
    )
    import matplotlib.pyplot as plt
...
def _annual_direct_emissions(results: Mapping[str, object]) -> np.ndarray:
    ...
    return _array(results["annual_output_th2"]) * _array(
        results["emissions_tco2_per_th2"]
    )
...
def main() -> None:
    """Run the hydrogen MACC generator from the command line."""

    parser = argparse.ArgumentParser(
        description="Generate a hydrogen marginal abatement cost curve."
    )
    parser.add_argument(
        "--project-root",
        type=Path,
    ...
    paths = generate_hydrogen_macc_outputs(
