"""Steel marginal abatement cost curve calculations.

The MACC compares each steel technology with the BF-BOF BAU baseline. The
abatement potential is direct stack-emissions abatement only:

    BF-BOF BAU direct emissions - technology direct emissions
...
All routes are compared at the same annual steel output, so their plotted
...
DEFAULT_MACC_TECHNOLOGIES = tuple(
    technology
    for technology in STEEL_MACC_TECHNOLOGY_LABELS
    if technology != "bf_bof_bau"
)
...
def deterministic_steel_macc(
    technologies: tuple[str, ...] = DEFAULT_MACC_TECHNOLOGIES,
) -> pd.DataFrame:
    """Calculate deterministic steel MACC values relative to BF-BOF BAU."""

    selected_technologies = ("bf_bof_bau", *technologies)
    results = calculate_deterministic_steel_results(
        technologies=selected_technologies,
    )
    return build_steel_macc_table(results)


def simulated_steel_macc(
    sample_size: int = DEFAULT_SAMPLE_SIZE,
    random_seed: int = DEFAULT_RANDOM_SEED,
    technologies: tuple[str, ...] = DEFAULT_MACC_TECHNOLOGIES,
    retrofit_bau_mode: str = DEFAULT_RETROFIT_BAU_MODE,
) -> pd.DataFrame:
    """Calculate steel MACC values from aligned Monte Carlo simulations.
...
def build_steel_macc_table(
    results_by_technology: Mapping[str, Mapping[str, object]],
) -> pd.DataFrame:
    """Build a steel MACC table from deterministic or simulated result mappings."""

    if "bf_bof_bau" not in results_by_technology:
        raise ValueError("results_by_technology must include 'bf_bof_bau'.")

    bau = results_by_technology["bf_bof_bau"]
        ...
        rows.append(
            {
                ...
                "label": STEEL_MACC_TECHNOLOGY_LABELS.get(technology, technology),
...
def plot_steel_macc(
    ...
    title: str = "Steel Sector - Marginal Abatement Cost Curve",
) -> object:
    """Plot a steel marginal abatement cost curve."""
    ...
    fig.text(
        0.99,
        0.02,
        "Each width is direct emissions avoided at 1 MtCS/year; routes are not additive. Costs include T&S and exclude carbon payments.",
        ha="right",
        fontsize=8,
        color="#555555",
    )
...
def generate_steel_macc_outputs(
    ...
    """Save one steel MACC CSV and PNG figure."""

    table = (
        deterministic_steel_macc(technologies=technologies)
        if deterministic
        else simulated_steel_macc(
            sample_size=sample_size,
            random_seed=random_seed,
    ...
    fig = plot_steel_macc(
        ...
        title=f"Steel Sector - Marginal Abatement Cost Curve ({mode.lower()})",
    )
    import matplotlib.pyplot as plt
...
def _annual_direct_emissions(results: Mapping[str, object]) -> np.ndarray:
    ...
    return _array(results["annual_output_tcs"]) * _array(
        results["emissions_tco2_per_tcs"]
    )
...
def main() -> None:
    """Run the steel MACC generator from the command line."""

    parser = argparse.ArgumentParser(
        description="Generate a steel marginal abatement cost curve."
    )
    parser.add_argument(
        "--project-root",
        type=Path,
